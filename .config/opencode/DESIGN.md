...
This document defines the visual design system for the project. All new components and pages
**must** follow these tokens, patterns, and conventions.
...
- **Styling:** Tailwind CSS v4 (CSS-first config via `@theme inline` in `globals.css` — no
  `tailwind.config.ts`)
...
All values use the **oklch** color space. Colors are defined as CSS custom properties in
...
Use these Tailwind utilities for status indicators — they are not part of the token system but are
used consistently:
...
```css
.auth-bg {
  background-image: radial-gradient(circle at 50% 0%, var(--accent) 0%, transparent 50%);
}
```
...
`Bot`, `User`, `Lock`, `Shield`, `Mail`, `Calendar`, `Copy`, `Check`, `Loader2`, `LogOut`, `Sun`,
`Moon`, `Github`, `ArrowLeft`, `RefreshCw`, `AlertCircle`, `FileQuestion`, `Database`, `Palette`,
`Video`
...
All components live in `src/components/ui/`. They use `data-slot` attributes, accept `className`
for overrides via `cn()`, and follow either `React.forwardRef` or functional component patterns.
...
Radix-based. Content: `rounded-md border p-1 shadow-md min-w-[8rem]`. Items support a `destructive`
variant.
...
- **Method:** Class-based via `next-themes` with `attribute="class"` and
