...
--     у conform;
--   * ширину, которую на экране съедает conceal, возвращает md-table/conceal.lua
--     виртуальным отступом - ТОЛЬКО на экране, файл всегда выровнен по сырой
--     ширине.
--
...
local defaults = {
  ...
  -- Компенсировать ли на экране разметку, скрытую conceal (см. md-table/conceal.lua):
...
-- Границы ячеек строки таблицы в байтах: { { from, to }, ... }, 1-based и
-- включительно, для участка МЕЖДУ трубами (вместе с окружающими пробелами).
-- Труба, экранированная слэшем, ячейку не делит - как и в split_row.
-- Нужна слою компенсации conceal, который работает с колонками, а не с текстом
...
function M.setup(opts)
  ...
  -- Экранная компенсация conceal: возвращает на экран ширину, которую съело
  -- скрытие разметки. Файла не касается
  require('md-table.conceal').setup(config, {
    cell_spans = cell_spans,
    strip = strip_markdown_syntax,
