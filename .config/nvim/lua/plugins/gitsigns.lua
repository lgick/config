...
local function update_statusline_color(buf)
  if not git_flow_active or not vim.api.nvim_buf_is_valid(buf) then
    return
  end
...
local function turn_off_git_mode()
  ...
  for _, buf in ipairs(all_bufs) do
    -- Проверяем, что буфер загружен и валиден
    if vim.api.nvim_buf_is_valid(buf) and vim.api.nvim_buf_is_loaded(buf) then
      ...
      if vim.b[buf].original_modifiable ~= nil then
        -- Сброс modifiable
        vim.bo[buf].modifiable = vim.b[buf].original_modifiable
        vim.b[buf].original_modifiable = nil
