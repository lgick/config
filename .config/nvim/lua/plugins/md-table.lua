...
require('md-table').setup({
  ...
  -- Neovim прячет инлайн-разметку (conceal), и таблица, ровная в файле, на
  -- экране разъезжается. 'auto' - md-table компенсирует это сам виртуальным
  -- отступом, если того же не делает render-markdown (pipe_table).
  -- Файла компенсация не касается.
